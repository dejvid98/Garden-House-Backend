...
exports.getAllItems = async (req, res) => {
  try {
    ...
    let fertilizerQuery = `SELECT name, speedup_time, COUNT(name) FROM fertilizer 
                          WHERE warehouse_id = $1
                          GROUP BY name,speedup_time`;
...
exports.getFertilizers = async (req, res) => {
  try {
    ...
    const fertilizerQuery = `SELECT name,COUNT(fertilizer.id),speedup_time,warehouse_id FROM fertilizer WHERE warehouse_id=$1
